...
export class ContactHandler {
	...
	private async processEmail(ctx: BotContext, userId: number, email: string): Promise<void> {
		...
		await ctx.services.questService.updateContact(userId, { email });
                const result = await ctx.services.questService.completeQuest(userId, "email_submit", email);
                await notifyReferralReward(ctx, result.referralReward);
        ...
        private async processWallet(ctx: BotContext, userId: number, wallet: string): Promise<void> {
			...
			return;
                ...
                const result = await ctx.services.questService.completeQuest(userId, "wallet_submit", wallet);
                await notifyReferralReward(ctx, result.referralReward);
